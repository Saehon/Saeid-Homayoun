...
CANDIDATES = {
    "C01": {"provider": "openai", "model": "gpt-6-astra"},
    "C02": {"provider": "google", "model": "gemini-3.8-flash"},
    "C03": {"provider": "anthropic", "model": "claude-fable-5"},
}
...
def main():
    ...
    pre = {
        ...
        "started_utc": utc_now(),
        "tasks_sha256": file_sha256(a.tasks),
        "evidence_sha256": file_sha256(a.evidence),
    ...
    for task in tasks:
        ...
        (run_dir / f"{task_id}.sha256").write_text(f"{digest}  {path.name}\n", encoding="utf-8")
    ...
    final = {
        **pre,
        ...
        "run_status": "EXECUTED" if all(r["status"] == "EXECUTED" for r in records) else "EXECUTED_WITH_FAILURES",
